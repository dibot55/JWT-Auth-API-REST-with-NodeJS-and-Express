...
export const signIn = async (req, res) => {
  ...
  try {
    ...
    // Validación de email
    const foundUser = await userSchema.findOne({email}).populate("roles");
    if(!foundUser){
    ...
    const matchPassword = await userSchema.comparePassword(password, foundUser.password); // true / false
    ...
    // Generar Token
    // variable igual al de signUp pero no son lo mismo
